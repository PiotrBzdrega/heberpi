...
#include <linux/compat.h>		/* for compat_uint_t, etc */
...
// prototype
static int doXlineIoctl( struct inode *inode_ptr, struct file *file_ptr, unsigned int command, unsigned long arg, char compat );
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	static inline void create_X10_sysfs_class( void )
	{
	  	X10_class = class_create( "Xline" );        //let ths sysfs export nasty stuff like pipe sizes
	}
...
/*====================================================================
 ...
 =====================================================================*/
	...
	// Entry point for 32-bit process accessing 32-bit driver OR 64-bit process accessing 64-bit driver
	...
	// Entry point for 32-bit process accessing 64-bit driver
	static long XlineIoctlCompat( struct file *file_ptr, unsigned int command, unsigned long arg )
	{
		unsigned long retValue;
		#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,39)
			mutex_lock( &xline_mutex );
	...
	{
		return doXlineIoctl( file_ptr->f_dentry->d_inode, file_ptr, command, arg, 0);

	}

#endif

// main IOCTL entry point -- kernel already locked at entry
static int doXlineIoctl( struct inode *inode_ptr, struct file *file_ptr, unsigned int command, unsigned long arg, char compat )
{
	...
	struct X10I_COMMAND user_params;
		...
		struct X10I_COMMAND_COMPAT user_params_compat;
	#endif
	...
	if ( command == IOCTL_X10I_COMMAND )
	{
		...
		if ( ( context->Type == X10i ) || ( context->Type == X15 ) )
		{
			...
			// need to translate params when 32-bit process calls into 64-bit driver

			#if defined( __x86_64__ )
				if(compat)
				{
					retValue = copy_from_user( &user_params_compat, (void *)arg, sizeof( struct X10I_COMMAND_COMPAT ) );
					user_params.error_code 			= compat_ptr( user_params_compat.error_code );
					user_params.virtual_pipe_number = user_params_compat.virtual_pipe_number;
					user_params.user_tx_buffer 		= compat_ptr( user_params_compat.user_tx_buffer );
					user_params.user_tx_buffer_size = (compat_long_t)user_params_compat.user_tx_buffer_size;
					user_params.user_rx_buffer   	= compat_ptr( user_params_compat.user_rx_buffer );
					user_params.user_rx_buffer_size = compat_ptr( user_params_compat.user_rx_buffer_size );
					user_params.command 			= user_params_compat.command;
				}
				else
			#endif
			{
...
static struct file_operations X10_file_operations = {
	...
	release:XlineRelease,	// int (*release)       ( struct inode *, struct file * );
		...
		// compat_ioctl is called when 32-bit user process calls into 64-bit driver
		compat_ioctl:XlineIoctlCompat,				// long (*ioctl)         ( struct file *, unsigned int, unsigned long );
	#else
		// older 'ioctl' pointer removed as of 2.6.36, replaced with 'unlocked_ioctl'
